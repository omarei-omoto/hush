...
| High | **`hush_add_secret` checked the platform before the policy.** On any host without native dialogs — every Linux box, and macOS under `HUSH_APPROVAL_MODE=file` — a scope `allowEnvs` forbids was never refused. The fallback message then handed the agent the exact command to run in the terminal to obtain the forbidden account | Policy first, capability second |
